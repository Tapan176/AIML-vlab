...
const MODEL_CODE = 'decision_tree';
...
export default function DecisionTree() {
    ...
    useEffect(() => {
        const cached = localStorage.getItem(`decision_tree_dataset`);
        if (cached) {
    ...
    const handleDatasetSelect = (data) => {
        ...
        if (data && data.filename) {
            localStorage.setItem(`decision_tree_dataset`, JSON.stringify(data));
        } else {
            localStorage.removeItem(`decision_tree_dataset`);
        }
    };
    ...
    const downloadCurrentImage = () => {
        ...
        link.href = imageUrl;
        link.download = `decision_tree_graph_${currentImageIndex + 1}.jpg`;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${constants.API_BASE_URL}/decision-tree`, {
                method: 'POST',
            ...
            {results && (
                <div className="download-section">
                    {(results.trained_model_drive_id || !results.session_id) && (
                        <DownloadTrainedModel selectedModel="decision_tree" extension=".pkl" sessionId={results.session_id} label="Download" />
                    )}
