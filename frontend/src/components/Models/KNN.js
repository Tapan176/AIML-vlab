...
export default function KNN() {
    ...
    useEffect(() => {
        const cached = localStorage.getItem(`knn_dataset`);
        if (cached) {
    ...
    const handleDatasetSelect = (data) => {
        ...
        if (data && data.filename) {
            localStorage.setItem(`knn_dataset`, JSON.stringify(data));
        } else {
    ...
    const downloadCurrentImage = () => {
        ...
        link.href = imageUrl;
        link.download = `knn_graph_${currentImageIndex + 1}.jpg`;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${constants.API_BASE_URL}/knn`, {
                method: 'POST',
            ...
            {results && (
                <div className="download-section">
                    {(results.trained_model_drive_id || !results.session_id) && (
                        <DownloadTrainedModel selectedModel="knn" extension=".pkl" sessionId={results.session_id} label="Download" />
                    )}
