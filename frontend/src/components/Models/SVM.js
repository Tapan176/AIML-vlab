...
export default function SVM() {
    ...
    useEffect(() => {
        const cached = localStorage.getItem(`svm_dataset`);
        if (cached) {
    ...
    const handleDatasetSelect = (data) => {
        ...
        if (data && data.filename) {
            localStorage.setItem(`svm_dataset`, JSON.stringify(data));
        } else {
            localStorage.removeItem(`svm_dataset`);
        }
    };
    ...
    const downloadCurrentImage = () => {
        ...
        link.href = imageUrl;
        link.download = `svm_graph_${currentImageIndex + 1}.jpg`;
            ...
            {results && (
                <div className="download-section">
                    {(results.trained_model_drive_id || !results.session_id) && (
                        <DownloadTrainedModel selectedModel="svm" extension=".pkl" sessionId={results.session_id} label="Download" />
                    )}
