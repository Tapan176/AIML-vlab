...
def supportVectorMachine(request, validated_params=None, user_id=None, session_version=None):
    data = request.json
    ...
    classifier = SVC(kernel=kernel, C=C, gamma=gamma, degree=degree, random_state=random_state)
    classifier.fit(X_train, y_train)
    model_path = saveTrainedModel(classifier, "support_vector_machine", "scikit-learn", user_id=user_id, version=session_version)
    ...
    outputImageUrls = [
        os.path.join(img_dir, 'supportVectorMachineTrainGraph.jpg'),
        os.path.join(img_dir, 'supportVectorMachineTestGraph.jpg')
    ]
