...
def randomForest(request, validated_params=None, user_id=None, session_version=None):
    data = request.json
    ...
    classifier = RandomForestClassifier(n_estimators=n_estimators, criterion=criterion, max_depth=max_depth, min_samples_split=min_samples_split, random_state=random_state)
    classifier.fit(X_train, y_train)
    model_path = saveTrainedModel(classifier, "random_forest", "scikit-learn", user_id=user_id, version=session_version)
    ...
    outputImageUrls = [
        os.path.join(img_dir, 'randomForestTrainGraph.jpg'),
        os.path.join(img_dir, 'randomForestTestGraph.jpg')
    ]
    ...
    return {
        ...
        "outputImageUrls": outputImageUrls, "trained_model_path": model_path,
