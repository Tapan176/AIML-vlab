"""Sentiment Analysis — TF-IDF + Logistic Regression pipeline."""
import pandas as pd
...
def train_sentiment_analysis(request, validated_params=None, user_id=None, session_version=None):
    ...
    pipeline = Pipeline([
        ('tfidf', TfidfVectorizer(max_features=p.get('max_features', 5000), ngram_range=(1, 2))),
        ('clf', LogisticRegression(max_iter=p.get('max_iter', 1000), C=p.get('C', 1.0))),
    ])
